// Independent downstream consumer via find_package(CollectiveScheduler CONFIG REQUIRED).
#include <collective_scheduler/scheduler.hpp>
#include <cstdio>
using namespace collective_scheduler;
static Participant mk(std::uint64_t id){ Participant p; p.id=ParticipantId(id); p.gen=ParticipantGeneration(1); p.worker=WorkerId(1); p.worker_boot=WorkerBootId(11); p.source=SourceId(1); p.source_boot=SourceBootId(100); p.device=DeviceId(id); p.node=NodeId(1); p.nic=NicId(id); p.capability_gen=CapabilityGeneration(1); p.health_gen=HealthGeneration(1); p.topology_gen=TopologyGeneration(1); p.ready=true; p.readiness_source=ProvenanceStatus::REAL; p.provenance=ProvenanceStatus::REAL; return p; }
int main(){
  Scheduler s;
  s.publishParticipant(mk(1)); s.publishParticipant(mk(2)); s.publishParticipant(mk(3));
  ...
  auto c2=submit(2,{ParticipantId(2),ParticipantId(3)},2);  // conflicts with C1 through B
  auto dec1=s.schedule();
  if (dec1.selected.size()!=1) { std::printf("CONSUMER FAIL: expected 1 selected\n"); return 1; }
  auto g1=s.grantById(dec1.grants[0]); if(!g1){ std::printf("CONSUMER FAIL: no grant\n"); return 1; }
  if(!s.handoff(*g1)){ std::printf("CONSUMER FAIL: handoff\n"); return 1; }
  if(!s.complete(*g1, WorkerBootId(11), SourceBootId(100))){ std::printf("CONSUMER FAIL: complete\n"); return 1; }
  auto dec2=s.schedule();
  if (dec2.selected.size()!=1 || dec2.selected[0].value!=c2.value){ std::printf("CONSUMER FAIL: second grant\n"); return 1; }
  auto g2=s.grantById(dec2.grants[0]); if(!g2||!s.handoff(*g2)){ std::printf("CONSUMER FAIL: handoff2\n"); return 1; }
  std::printf("CONSUMER OK: deterministic order, conflict serialized, C1 completed, C2 granted\n");
  return 0;
}